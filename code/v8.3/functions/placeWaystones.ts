...
world.afterEvents.itemUse.subscribe(({source: player, itemStack: item}) => {
  ...
  .show(player).then(({canceled, selection}) => {
    ...
    const direction = offsetDirection[rot > -45 && rot < 45 ? "South" : rot < -135 || rot > 135 ? "North" : rot > -135 && rot < -45 ? "East" : "West"]
    for(let i = group[0]; i < group[1] +1; i++){
      const id = waystonesListId[i]
      const multiply = i - group[0] +1
      const block = player.dimension.getBlock({x: player.location.x + (direction.x * multiply), y: player.location.y + (direction.y * multiply), z: player.location.z + (direction.z * multiply)})
